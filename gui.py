...
class GameOfLifeGUI:
    ...
    def create_buttons(self):
        ...
        button_step = tk.Button(button_frame, text='Next step', command=self.button_func)
        button_rand = tk.Button(button_frame, text='Randomize', command=self.button_random)
        button_sim =  tk.Button(button_frame, text='Simulate', command=self.button_simulate)
        button_clear = tk.Button(button_frame, text='Clear', command=self.button_clearf)
        button_stop = tk.Button(button_frame, text='Stop', command=self.button_stopf)
    ...
    def button_stopf(self):
        self.simulation_running = False

    def button_random(self):
        self.current_board = create_random_board(self.cols, self.rows)
        self.render(self.current_board)

    def button_clearf(self):
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if (self.current_board[row][col] == 1):
                    ...
                    fill_color = 'white'
                rectangle_id = self.canvas.create_rectangle(x, y, x+self.cell_width, y+self.cell_height, width=1, outline='gray', fill=fill_color)
                self.rectangle_id_to_cell[rectangle_id] = (row, col)
                self.canvas.tag_bind(rectangle_id, '<Button-1>', lambda e, rect_id=rectangle_id: self.on_rectangle_click(rect_id))
